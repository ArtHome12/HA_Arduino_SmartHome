...
const int cyclesPowerLowLimit = 12;     // Предел для счётчика циклов.
...
void setup()
{
  ...
  // Восстановим из EEPROM информацию о начале завершения работы. Используем именно единицу, как признак.
  if (EEPROM.read(eepromAddrShutdown) == 1)
    powerOffTimer = 1;
...
void sendShutdown() {
  ...
  // Если это включение таймера (первое увеличение), запишем информацию в EEPROM.
  if (!powerOffTimer)
    EEPROM.write(eepromAddrShutdown, 1);

  // Включаем либо увеличиваем таймер отключения питания.
  powerOffTimer++;

  // Включаем светодиод для индикации, что RPi завершает работу, но только если 
  // RPi ещё не выключена и светодиод не был погашен.
//  if (!RPiTurnedOff)
//    digitalWrite(LED_BUILTIN, HIGH);
}
...
void powerControl(int voltage, int power){
  ...
  if (voltage > mVoltageHiBound && powerOffTimer > 0) {
    ...
    // Заново запускаем счётчик низкого энергопотребления.
    cyclesPowerLow = 0;
